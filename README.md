...
### Feed流
什么是feed流？
关注推送也叫feed流，也就是投喂，为用户持续提供沉浸式的体验，通过无限下拉刷新获取新的信息。Feed流通过算法和用户行为数据分析，动态地将用户感兴趣的内容呈现在用户的界面上。

常见模式是按照时间排序、智能排序

朋友圈就是时间排序，优点是信息全面，无损失，实现简单。缺点是信息噪音多，用户不一定感兴趣，内容获取的效率低。

智能排序就是用智能算法屏蔽掉违规的、用户不感兴趣的内容。

优点是投喂用户感兴趣的信息，用户粘度增高，容易沉迷，缺点是如果推送不精准会起到反效果。

我们基于关注的好友做Feed流。用Timeline模式。

![image](https://i-blog.csdnimg.cn/blog_migrate/5852c7bc021c5311b41d666e04d3353c.png)
